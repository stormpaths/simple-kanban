...
async def test_board(test_user):
    ...
    async with TestSessionLocal() as session:
        board = Board(
            name="Test Board", description="A test board", owner_id=test_user.id
        )
        session.add(board)
        await session.commit()
...
class TestUserModel:
    ...
    async def test_create_user(self, setup_database):
        ...
        async with TestSessionLocal() as session:
            user = User(
                username="newuser", email="newuser@example.com", full_name="New User"
            )
...
class TestBoardModel:
    ...
    async def test_create_board(self, setup_database, test_user):
        ...
        async with TestSessionLocal() as session:
            board = Board(
                name="My Board", description="A test board", owner_id=test_user.id
            )
    ...
    async def test_board_owner_relationship(self, setup_database, test_user):
        ...
        async with TestSessionLocal() as session:
            board = Board(name="Test Board", description="Test", owner_id=test_user.id)
            session.add(board)
            await session.commit()

            # Test relationship loading
            result = await session.execute(select(Board).where(Board.id == board.id))
...
class TestColumnModel:
    ...
    async def test_column_board_relationship(self, setup_database, test_board):
        ...
        async with TestSessionLocal() as session:
            ...
            # Test relationship loading
            result = await session.execute(select(Column).where(Column.id == column.id))
...
class TestTaskModel:
    ...
    async def test_task_column_relationship(self, setup_database, test_board):
        ...
        async with TestSessionLocal() as session:
            ...
            task = Task(
                title="Test Task", description="Test", position=1, column_id=column.id
            )
            session.add(task)
            await session.commit()
