...
def create_test_user(base_url: str) -> dict:
    ...
    try:
        response = requests.post(
            f"{base_url}/api/auth/register", json=payload, timeout=10
        )
...
def save_credentials(credentials: dict, output_dir: Path):
    ...
    with open(env_file, "w") as f:
        f.write(
            f"# Test user credentials - Generated {time.strftime('%Y-%m-%d %H:%M:%S')}\n"
        )
...
def main():
    ...
    parser.add_argument(
        "--verify", action="store_true", help="Verify credentials after creation"
    )
    ...
    if args.verify:
        if not verify_credentials(
            args.base_url, credentials["email"], credentials["password"]
        ):
    ...
    print(
        f"   Python import:      from test_credentials import TEST_USERNAME, TEST_PASSWORD"
    )
    print()
    print("🔑 Credentials:")
