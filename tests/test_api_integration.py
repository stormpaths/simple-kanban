...
async def authenticated_user():
    ...
    return {
        "user": user,
        "token": token,
        "headers": {"Authorization": f"Bearer {token}", "X-CSRF-Token": "test_csrf_token_12345678"}
    }
...
class TestBoardAPI:
    ...
    @pytest.mark.asyncio
    async def test_get_board_with_columns_and_tasks(self, setup_database, authenticated_user):
        ...
        response = client.get(f"/api/boards/{board_id}", headers=authenticated_user["headers"])
...
class TestCompleteWorkflows:
    ...
    async def test_error_handling_workflow(self, setup_database, authenticated_user):
        ...
        response = client.get("/api/boards/99999", headers=authenticated_user["headers"])
        assert response.status_code == 404
