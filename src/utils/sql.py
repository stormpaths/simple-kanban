...
class SQLQueryBuilder:
        ...
        if eager_load:
            for relationship in eager_load:
                if hasattr(model_class, relationship):
                    query = query.options(
                        selectinload(getattr(model_class, relationship))
                    )
    ...
    async def insert_one(self, model_class, data: Dict[str, Any]) -> Any:
        """
        ...
        valid_fields = {
            key: value for key, value in data.items() if hasattr(model_class, key)
        }
    ...
    async def update_one(
        self, model_class, where_conditions: Dict[str, Any], update_data: Dict[str, Any]
    ) -> bool:
        """
    ...
    async def delete_one(self, model_class, where_conditions: Dict[str, Any]) -> bool:
        """
