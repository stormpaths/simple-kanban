...
class OIDCConfigManager:
    ...
    def _load_configurations(self):
        ...
        stormpath_config = self._load_from_secret(
            "/var/secrets/stormpath/google-auth.json"
        )
        if stormpath_config:
        ...
        if kanban_config:
            self._providers["google"] = self._create_google_config(
                kanban_config, is_kanban_specific=True
            )
    ...
    def _create_google_config(
        self, config_data: Dict[str, Any], is_kanban_specific: bool = False
    ) -> OIDCProviderConfig:
